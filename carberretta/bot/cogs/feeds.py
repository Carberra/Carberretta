...
class Feeds(commands.Cog):
    ...
    async def get_new_vods(self) -> str:
        current_vod = await self.bot.db.field("SELECT Vod FROM videos WHERE ID = 1")
        ...
        for item in await self.call_feed():
            ...
            if current_vod == item.yt_videoid:
                ...
                await self.bot.db.execute("UPDATE videos SET Vod = ? WHERE ID = 1", item.yt_videoid)
                return item.yt_videoid

    async def get_new_videos(self) -> str:
        current_vid = await self.bot.db.field("SELECT Vid FROM videos WHERE ID = 1")
        ...
        for item in await self.call_feed():
            ...
            if item.yt_videoid == current_vid:
                ...
                if "#VOD" not in item.summary:
                    ...
                    await self.bot.db.execute("UPDATE videos SET Vid = ? WHERE ID = 1", item.yt_videoid)
                    return item.yt_videoid

    async def get_new_premieres(self) -> tuple:
        known_premieres = {
            _id: [_upcoming, _announced] for _id, _upcoming, _announced in await self.bot.db.records("SELECT * FROM premieres")
        }
