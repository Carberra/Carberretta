...
class Experience(commands.Cog):
    ...
    async def command_level(self, ctx: commands.Context, member: typing.Optional[discord.Member]) -> None:
        member = member or ctx.author
        exp = await self.bot.db.field("SELECT Experience FROM members WHERE MemberID = ?", member.id)
        if exp is None:
    ...
    async def command_leveltop(self, ctx: commands.Context):
        leaderboard = await self.bot.db.records(
            "SELECT MemberID, Level, Experience FROM members ORDER BY Experience DESC LIMIT 10")

        embed = discord.Embed.from_dict({
            "title": "Experience Leaderboard",
            "color": DEFAULT_EMBED_COLOUR,
            "description": "\n".join(
                [f"{i + 1}. {self.bot.get_member(member[0]).display_name}, level `{member[1]}`, experience `{member[2]}`." for
                 i, member in enumerate(leaderboard)]),
            "thumbnail": {"url": f"{self.bot.get_member(leaderboard[0][0]).avatar_url}"},
