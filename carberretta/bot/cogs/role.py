...
class Role(commands.Cog):
    ...
    async def command_create(self, ctx, stack: bool, channel: discord.TextChannel, message: str, *, selection: str) -> None:
        roles = []
        for i in (j := iter(selection.split(" "))):
            roles.append((
                i,
                discord.utils.get(ctx.guild.roles, mention=next(j))
            ))
                    ...
                    if not data["stack"]:
                        for emoji in (reaction.emoji for reaction in message.reactions if reaction.emoji != payload.emoji.name):
                            try:
                                ...
                                raise
                        await payload.member.remove_roles(*data["roles"].values(), reason=f"Removed due to role reaction to message {payload.message_id}")

                    await payload.member.add_roles(data["roles"].get(f"{payload.emoji}"), reason=f"Added due to role reaction to message {payload.message_id}")
                except:
    ...
    async def on_raw_reaction_remove(self, payload) -> None:
        if roles := next((self._cache[m]["roles"] for m in self._cache if m.id == payload.message_id), None):
            if not (member := self.bot.get_guild(payload.guild_id).get_member(payload.user_id)).bot:
                try:
                    await self.bot.get_guild(payload.guild_id).get_member(payload.user_id).remove_roles(roles.get(f"{payload.emoji}"), reason=f"Removed due to role reaction to message {payload.message_id}")
                except:
